...
class AnimatedLED:
    def __init__(self, config):
        ...
        try:
            ...
            logging.exception(f"Error initializing neopixels:\n{e}\n\n{traceback.format_exc()}")
            os._exit(1)
    ...
    def write(self, forceUpdate):
        ...
        for sectionState in self.states:
            currentBrightness = ANIM_FUNCTIONS[sectionState.anim](time.time() / sectionState.animInterval)
            brightnesses.append(currentBrightness)

            sectionLen = (sectionState.bounds[1] - sectionState.bounds[0]) if sectionState.bounds[1] is not None else len(self.leds)
            mixedColor = [(prim * currentBrightness[0] + sec * currentBrightness[1]) for prim, sec in zip(sectionState.rgb, sectionState.secondaryRgb)]
            self.leds[sectionState.bounds[0]:sectionState.bounds[1]] = [mixedColor] * sectionLen

        # Check whether brightnesses have changed
        if not forceUpdate and self.brightnesses is not None and all([b[0] == b[1] for b in zip(brightnesses, self.brightnesses)]):
            return
...
class LEDState:
    def __init__(self, bounds = (0, None), rgb = (0, 0, 0), secondaryRgb = (0, 0, 0), anim = "solid", animInterval = 1):
        self.bounds = bounds
        self.rgb = rgb
