...
class StatusLEDConfig(configparser.ConfigParser):
    ...
    def load(self):
        ...
        self.parsedStates = [{"config": self[section]} for section in self.sections() if section.split(" ")[0] == "state"]
        ...
        for state in self.parsedStates:
            ...
            logging.info(f"Parsed state: {state['config'].name}")

        self.parsedSections = [{"config": self[section]} for section in self.sections() if section.split(" ")[0] == "section"]
        ...
        for section in self.parsedSections:
            ...
            logging.info(f"Parsed section: {section['config'].name}")


    def getLEDStateBySection(self, currentState):
        logging.info(f"Loading state config of '{currentState}'")
        ...
        for section in ([{}] + self.parsedSections):
            ...
            for state in self.parsedStates:
                if  (
                    ...
                    if "sectionNameList" in state and "sectionName" in section and section["sectionName"] not in state["sectionNameList"]:
                        continue
            ...
            if stateOfThisSection:
                states.append(LEDState(
                    StatusLEDConfig.strToIntTuple(section["config"].get("bounds")) if section else (0, None),
                    StatusLEDConfig.strToColor(stateOfThisSection["config"].get("rgb")),
                    StatusLEDConfig.strToColor(stateOfThisSection["config"].get("secondary_rgb", fallback="0, 0, 0")),
                    stateOfThisSection["config"].get("animation", fallback="solid"),
                    float(stateOfThisSection["config"].get("animation_interval", fallback=1))
                ))
            else:
                fallbackBounds = (0, None)
                fallbackColor = (0, 0, 0)
                if "fallback_rgb" in self["status_led"]:
                    fallbackColor = StatusLEDConfig.strToColor(self.get("status_led", "fallback_rgb"))
                if section:
                    if "fallback_rgb" in section["config"]:
                        fallbackBounds = StatusLEDConfig.strToIntTuple(section["config"].get("bounds"))
                        fallbackColor = StatusLEDConfig.strToColor(section["config"].get("fallback_rgb"))
                states.append(LEDState(
                    fallbackBounds,
                    fallbackColor
                ))
